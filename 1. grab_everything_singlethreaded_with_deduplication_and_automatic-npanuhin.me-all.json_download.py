...
# Change SCRIPT_DIR to the execution/installation directory of this pipeline
...
# Standardized client identification payload to minimize remote connection drops
headers = {
...
def inject_metadata_iptc(file_path, title, description, copyright_text):
    """Injects contextual EXIF/IPTC metadata tags optimized for Windows Explorer shell indexing."""
    try:
...
def download_single_wallpaper(task):
    """Downloads an individual asset, extracts metadata, evaluates deduplication, and injects shell properties."""
...
def main():
    ...
    for short_name, json_key in REGIONS.items():
        ...
        for entry in db[actual_key]:
            ...
            # 1. Isolate the original Microsoft/Bing internal codename
            base_key = None
            try:
            ...
            # 2. Map localized properties into clean target metadata definitions
            img_name = None
            ...
            # Engine Block E: Split mashed CamelCase strings and expand condensed conjoined tags
            if not img_name or img_name.lower() in ["bingimage", "wallpaper", "image"]:
                if base_key and not base_key.startswith("date_"):
                    ...
                    for kw in bing_keywords:
                        working_name = re.sub(f'(?<=[A-Za-z])({kw})(?=[A-Za-z])|(?<=[A-Za-z])({kw})$|'
                                              f'^({kw})(?=[A-Za-z])', r' \1\2\3 ', working_name, flags=re.IGNORECASE)
                    ...
                    # FIX: Corrected the lookbehind syntax layout to (?<=\d) to resolve the syntax error
                    working_name = re.sub(r'(?<=[A-Za-z])(?=\d)|(?<=\d)(?=[A-Za-z])', ' ', working_name)
                    img_name = clean_title(working_name)
                    
            # Priority 5 (THE ULTIMATE PREMIUM BACKUP): Native character and script preservation logic
            ...
            # 3. Cross-Region Deduplication Logic Engine with explicit quality and multi-turn checks
            if base_key not in unique_wallpapers:
                ...
                # UNICODE PRESERVATION VECTOR: Prevent rich descriptive Asian titles from being replaced by generic western assets
    ...
    print(f"[*] Starting single-threaded downloads (Safe mode with EXIF/IPTC injection)...\n")
